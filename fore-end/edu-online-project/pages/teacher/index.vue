      ...
      <section class="c-sort-box unBr">
        <div>
          ...
          <article class="i-teacher-list" v-if="data.total > 0">
            <ul class="of">
              <li v-for="(teacher,index) in data.teacherList" :key="index">
...
export default {
    //asyncData 相当于 created ,在加载页面前调用后台接口获取数据渲染页面
    // asyncData({ params, error }){
    //     return axios.get('http://localhost:8001/edu/teacher/getTeacherList')
    //     .then(response => {
    ...
    // }
  asyncData({params,error}){
    return teacher.getFrontTeacherInfo(1,8)
    .then(response => {
  ...
  methods: {
    gotoPage(page){
      teacher.getFrontTeacherInfo(page,8)
      .then(response => {
